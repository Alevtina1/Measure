...
class Matrix {
  ...
  mulVector (vector) {
    const resultVector = Array(this.size).fill(0);

    _.forEach(this._data, (row, index) => {
      resultVector[index] = Object.keys(row)
        .reduce((acc, column) => acc + row[column]*vector[column], 0);
    });

    return resultVector;
  }

  maxEigenVector (verbose=false, accuracy=1e-12) {
    let firstVector = Array(this.size).fill(0);
    let nextVector = Array(this.size).fill(1);
    let maxDifferent = 0;

    const floatEquals = (a, b, d=accuracy) => {
      return Math.abs(a - b) <= d;
    };

    const normVector = (vector) => {
      const vectorLength = Math.sqrt(vector.reduce((acc, coord) => acc + coord*coord, 0));
      return vector.map((coord) => coord/vectorLength);
    };

    const vectorEquals = (v, u) => {
      maxDifferent = 0;
      return v.reduce((acc, coord, i) => {
        maxDifferent = Math.max(maxDifferent, Math.abs(v[i] - u[i]));
        return (acc && floatEquals(v[i], u[i]) ? true : false);
      }, true);
    };

    while (!vectorEquals(firstVector, nextVector)) {
      if (verbose) {
        console.log(`Find max eigen vector: MAX DIFFERENCE: ${maxDifferent}`);
      }
      firstVector = _.cloneDeep(nextVector);
      nextVector = normVector(this.mulVector(firstVector));
    }

    return nextVector;
  }

  eigenValueByVector (vector) {
    const vectorImage = this.mulVector(vector);
    return vectorImage[0]/vector[0];
  }
}
